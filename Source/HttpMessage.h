...
namespace Http {
    ...
    public:
        CRequest(std::shared_ptr<CSession>&& session,
            beast::http::request<beast::http::string_body>&& request)
            : IRequest(std::move(session)), mImpl(std::move(request)) {}
        ...
        auto bodyData() const -> const char* override {
            return mImpl.body().c_str();
        }
