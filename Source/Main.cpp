...
private:
  ...
  void on_read(beast::error_code ec, std::size_t bytes_transferred) {
    ...
    ws_.async_write(mFlatBuffer.data(),
...
public:
  ...
  http_session(tcp::socket &&socket,
      ...
      : mStream(std::move(socket)), doc_root_(doc_root) {
  ...
  void run() {
    ...
    net::dispatch(mStream.get_executor(),
...
private:
  ...
  void on_read(beast::error_code ec, std::size_t bytes_transferred) {
    ...
    if (websocket::is_upgrade(mRequestParser->get())) {
      ...
      std::make_shared<websocket_session>(mStream.release_socket())
          ->do_accept(mRequestParser->release());
      return;
    }
...
int main(int argc, char *argv[]) {
    ...
    try
    {
        ...
        server->addRoute(beast::http::verb::get, "/hello/{}/world", [=](Http::CSession* session, beast::http::request<beast::http::string_body>&& request, std::string a) {
            session->sendResponse(Http::InternalServerError(std::move(request), "world!"));
        });
