...
namespace Http {
    ...
    public:
        ...
        void run() {
            asio::dispatch(
                ...
                beast::bind_front_handler(&CSession::doRead, this->shared_from_this()));
        }

        template <class Body, class Allocator>
        void sendResponse(
            beast::http::response<Body, beast::http::basic_fields<Allocator>>&& req) {
            asio::dispatch(mStream.get_executor(),
                beast::bind_front_handler(&CSession::queueWrite,
    ...
    protected:
        ...
        void onWrite(bool keep_alive, beast::error_code errorCode,
        ...
        static beast::http::message_generator handleRequest(
            ...
            if (ec == beast::errc::no_such_file_or_directory)
                return NotFound(std::move(req), "The resource '" + std::string(req.target()) +
                    "' was not found.");

            if (ec)
                return InternalServerError(std::move(req),
